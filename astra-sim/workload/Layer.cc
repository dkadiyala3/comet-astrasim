...
void Layer::call(EventType event, CallData* mdata) {
  if (event == EventType::Wight_Grad_Comm_Finished) {
    ...
    // Divya's Debug
    //std::cout <<"Registering wegiht_Grad_finished_after_delay for layer "<<
    //    this->id << " at time: "<<last_wg_finished<<std::endl;
...
void Layer::issue_weight_grad_comm(
  ...
  // std::cout <<"setting wg->set_notifier for wg: "<<wg->my_id<<std::endl; //Divya's Debug
